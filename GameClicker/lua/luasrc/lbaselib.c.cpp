...
static int luaB_pairs (lua_State *L) {
    ...
    static std::function<int(lua_State*)> f(luaB_next);
    return pairsmeta(L, "__pairs", 0, &f);
}
...
{
    static std::function<int(lua_State*)> f1(ipairsaux);
    static std::function<int(lua_State*)> f2(ipairsaux_raw);
...
LUAMOD_API int luaopen_base (lua_State *L) {
  ...
  static std::function<int(lua_State*)> f1(luaB_type);
  lua_pushcclosure(L, &f1, LUA_NUMTAGS);
